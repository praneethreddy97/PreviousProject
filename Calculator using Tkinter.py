...
for x in range(3):
    for y in range(2):
        button= Button(root, text=operations[count],width=8, height=2, command= lambda text=operations[count]:get_operation(text))
        count=count+1
...
Button(root,text="Delete",width=8,height=2,command= undo).grid(row=5,column=3)
